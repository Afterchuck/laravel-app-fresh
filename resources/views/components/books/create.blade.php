        ...
        <div class="rounded-3xl border border-slate-200 p-8 shadow-sm">
            <form action="{{ route('books.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="flex items-center gap-3 pt-2">
                    <button type="submit" class="rounded-lg bg-indigo-900 px-5 py-3 text-sm font-semibold text-white hover:bg-slate-800">Save Book</button>
                    <a href="{{ route('books.index') }}" class="text-sm font-semibold text-white hover:text-slate-900">Cancel</a>
                </div>
            </form>
        </div>
    </div>
